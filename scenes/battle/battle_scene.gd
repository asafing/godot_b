...
func _ready() -> void:
	...
	player_mons.push_front($PlayerSpots/VBoxContainer/Panel1/PlayerTopSpot.get_occupying_mon())
	player_mons.push_front($PlayerSpots/VBoxContainer/Panel2/PlayerTopSpot.get_occupying_mon())
	enemy_mons.push_front($EnemySpots/VBoxContainer/Panel1/EnemyTopSpot.get_occupying_mon())
	enemy_mons.push_front($EnemySpots/VBoxContainer/Panel2/EnemyBotSpot.get_occupying_mon())
...
func on_action_select(action_index: int) -> void:
	print(
		(
			"Turn %d: Action %d selected for monster indexed %d"
			% [current_turn, action_index, active_mon_index]
		)
	)
	chosen_action_indexes.push_front(action_index)
	chosen_action_target_indexes.push_front(0)
...
func _on_action_complete() -> void:
	var target_mon: Mon = enemy_mons[chosen_action_target_indexes[current_action_playing_index]]
	player_mons[current_action_playing_index].inflict_damage(
		target_mon, current_action_playing_index
	)
	current_action_playing_index += 1
	if current_action_playing_index >= len(chosen_action_indexes):
		start_post_turn()
	else:
		player_mons[current_action_playing_index].play_action_towards(
			enemy_mons[0], chosen_action_indexes[current_action_playing_index]
		)
